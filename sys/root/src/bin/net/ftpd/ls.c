...
char **sreaddir( ftp_session_t *session, const char *dirname )
{
	...
	if (combine_path(session->wd, dirname, session->u_session.u_path.combpath, sizeof(session->u_session.u_path.combpath)) < 0 ||
			get_full_path(session, session->u_session.u_path.combpath, session->u_session.u_path.fullpath, sizeof(session->u_session.u_path.fullpath)) < 0)
		return NULL;
	...
	isroot = session->u_session.u_path.fullpath[0] == '/' && session->u_session.u_path.fullpath[1] == '\0';
	if (isroot) {
...
berkeley:
		...
		while ( (de = readdir( d ) ) != NULL ) {
			...
			if (combine_path(session->wd, de->d_name, session->u_session.u_path.combpath, sizeof(session->u_session.u_path.combpath)) == 0 &&
					get_full_path(session, session->u_session.u_path.combpath, session->u_session.u_path.fullpath, sizeof(session->u_session.u_path.fullpath)) == 0 &&
					stat(session->u_session.u_path.fullpath,&st) == 0) {
				s -= sizeof(int);
...
void donlist( ftp_session_t *session, char *arg )
{
	...
	session->opt_l = session->opt_a = session->opt_C = session->opt_d = session->opt_F = session->opt_R = session->opt_r = session->opt_t = session->opt_S = 0;
	...
	close( c );
	if ( session->opt_a || session->opt_C || session->opt_d || session->opt_F || session->opt_l || session->opt_r || session->opt_R ||
