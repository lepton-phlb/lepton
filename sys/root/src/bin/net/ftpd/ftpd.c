...
{
	if ( session->loggedin || 
			(session->server->check_pwd == 0 || (*session->server->check_pwd)(session->account, password) == 0)) {
...
void docwd( ftp_session_t *session, const char *dir )
{
	...
	if (combine_path(session->wd, dir, session->u_session.u_path.combpath, sizeof(session->u_session.u_path.combpath)) < 0 ||
			get_full_path(session, session->u_session.u_path.combpath, session->u_session.u_path.fullpath, sizeof(session->wd)) < 0 ||
			((session->u_session.u_path.fullpath[0] != PATH_SEPARATOR_CHAR || session->u_session.u_path.fullpath[1] != '\0') &&
...
void dopasv( ftp_session_t *session, int useepsv )
{
	...
	if ( session->server->firstport && session->server->lastport )
		firstporttried = session->server->firstport + ( session->thread_hdl % (session->server->lastport-session->server->firstport+1) );
	else
		firstporttried = 0;
...
{
	if (combine_path(session->wd, name, session->u_session.u_path.combpath, sizeof(session->u_session.u_path.combpath)) < 0 ||
			get_full_path(session, session->u_session.u_path.combpath, session->u_session.u_path.fullpath, sizeof(session->u_session.u_path.fullpath)) < 0) {
...
int get_full_path(ftp_session_t *session, const char *path, char *result_path, int result_path_size) {
