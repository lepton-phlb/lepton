...
int kernel_timer_create(clockid_t clockid, struct sigevent * p_sigevent,kernel_timer_t* p_kernel_timer)
{
	...
	//set sigevent
	if(!p_sigevent){
		memcpy((void *)&p_kernel_timer->sigevent,(void *)&default_timer_sigevent,sizeof(struct sigevent));
	}else{
   ...
   //
	...
	if(p_kernel_timer->itimerspec.it_value.tv_nsec!=0 || p_kernel_timer->itimerspec.it_value.tv_sec!=0){
	   ...
	   cyg_alarm_initialize(p_kernel_timer->timer.alarm_hdl,
						__get_timer_ticks() + msec_to_tick((__timer_s_to_ms(value->it_value.tv_sec)+__timer_ns_to_ms(value->it_value.tv_nsec))),
						msec_to_tick((__timer_s_to_ms(value->it_interval.tv_sec)+__timer_ns_to_ms(value->it_interval.tv_nsec))));
#endif
	}
