...
void _init_syscall(void)
{
	...
	if (0 != cyg_hal_sys_sigprocmask(CYG_HAL_SYS_SIG_UNBLOCK, &blocked, (cyg_hal_sys_sigset_t*) 0)) {
	...
	if (0 != cyg_hal_sys_sigaction(KERNEL_INTERRUPT_NB,  &action, (struct cyg_hal_sys_sigaction*) 0)) {
...
void _kernel_syscall_handler_synth(int sig, cyg_hal_sys_siginfo_t * info, void *ptr)
{
	...
	//thread en mode user
			...
			kh_context->hal_uc_mcontext.hal_gregs[S_REG_ESP] = (int)(pthread_ptr->kernel_stack + KERNEL_STACK
					- 2*sizeof(int));
	}
}
				...
				cyg_addrword_t info)
{
	...
	else if(pthread_ptr->stat ==  PTHREAD_STATUS_NULL || (pthread_ptr->stat&PTHREAD_STATUS_SIGHANDLER) == PTHREAD_STATUS_SIGHANDLER) {
			//changement de mode
		__kernel_begin_syscall(pthread_ptr)
...
void _kernel_syscall_handler(void) {
	...
	if((pthread_ptr->stat&PTHREAD_STATUS_KERNEL) == PTHREAD_STATUS_KERNEL) {
		...
		memcpy((void *)pthread_ptr->bckup_context.user_stack_addr, (void *)pthread_ptr->bckup_context.svc_regs, sizeof(svc_reg_t));
		...
		*(pthread_ptr->bckup_context.ret_addr+2)=pthread_ptr->bckup_context.user_stack_addr+sizeof(svc_reg_t);
	}
	/*thread en mode user (1er passage dans la routine)*/
	else if(pthread_ptr->stat ==  PTHREAD_STATUS_NULL || (pthread_ptr->stat&PTHREAD_STATUS_SIGHANDLER) == PTHREAD_STATUS_SIGHANDLER) {
		//changement de mode
		__kernel_begin_syscall(pthread_ptr)
		//align 8 bytes
		pthread_ptr->bckup_context.kernel_stack_addr = (unsigned int)(pthread_ptr->kernel_stack+KERNEL_STACK);
		...
		memcpy((void *)pthread_ptr->bckup_context.kernel_stack_addr, (void *)pthread_ptr->bckup_context.svc_regs, sizeof(svc_reg_t));
		...
		pthread_ptr->bckup_context.svc_regs = (svc_reg_t*)pthread_ptr->bckup_context.kernel_stack_addr;
		...
		pthread_ptr->bckup_context.user_stack_addr = (unsigned int)(pthread_ptr->bckup_context.svc_r12);
		//modify stack
		__asm__("ldr r4,%0\n\t"
				"msr psp,r4\n\t"
				:
