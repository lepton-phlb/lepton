...
pid_t _sys_vfork(kernel_pthread_t* pthread_ptr){
   ...
   memset((void *)process_lst[pid],0,sizeof(process_t));

   //copy all ppid father process properties to the son (pthread_ptr, ...)
   memcpy((void *)process_lst[pid],(void *)process_lst[ppid],sizeof(process_t));
   ...
   //not command line yet
   process_lst[pid]->argv[argc]=(char *)strtok(process_lst[pid]->arg," ");
   while(process_lst[pid]->argv[argc++]){
      process_lst[pid]->argv[argc]=(char *)strtok(0," ");
   }
   process_lst[pid]->argc=argc-1;


   //2)pthread operation
   backup_parent_pthread_ptr=(kernel_pthread_t*)malloc(sizeof(kernel_pthread_t));
   ...
   //phlb bug fix 3.0.0.8 process_lst[pid]->pthread_ptr->next = (kernel_pthread_t*)0; //(not need kernel_put_pthread_id() and kernel_get_pthread_id() operation)
...
int _sys_vfork_exit(kernel_pthread_t* pthread_ptr,int status){
   ...
   //from phlb bug fix 3.0.0.5
   //backup_parent_pthread_ptr->next  = pthread_ptr->next;

   //unswicth pthread_ptr
   memcpy((void *)pthread_ptr,(void *)backup_parent_pthread_ptr,sizeof(kernel_pthread_t));
   process_lst[pid]->pthread_ptr = backup_parent_pthread_ptr;

   //from phlb bug fix 3.0.0.5 : re-insert pthread from the pthread list of father process
   ...
   memset((void *)process_lst[pid]->pthread_ptr,0,sizeof(kernel_pthread_t));
...
|
| Revision 1.2  2009/06/23 13:17:12  jjp
