...
#define TWI_WSDA(__SDA_VALUE__) (__SDA_VALUE__?((*AT91C_PIOA_SODR)=(1<<7)):((*AT91C_PIOA_CODR)=(1<<7)) )
...
----------------------------------------------*/
int _twi_io_write(unsigned char dev_addr,const unsigned char* data,unsigned int data_len,unsigned char addr_size)
{
  char ack;
...
| Comments:  Pour la programmation de l'EEPROM verifier que \WC = 0 
----------------------------------------------*/
int _twi_io_read(unsigned char dev_addr,unsigned char* data,unsigned int data_len,unsigned char addr_size)
{
  char ack;
  int i;
