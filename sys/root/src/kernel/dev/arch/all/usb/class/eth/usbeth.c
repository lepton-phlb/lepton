...
static const usb_configuration_descriptor usb_configuration_eth = {
      ...
      configuration_id:   1,
      configuration_str:  0,
      attributes:         (USB_CONFIGURATION_DESCRIPTOR_ATTR_REQUIRED | USB_CONFIGURATION_DESCRIPTOR_ATTR_SELF_POWERED),
      max_power:          0//50
};
...
int dev_usbeth_isset_read(desc_t desc){
	...
	return ofile_lst[ofile_lst[desc].desc_nxt[0]].pfsop->fdev.fdev_isset_read(USB_ENDPOINT_DESCRIPTOR_ETH_RX_NO);
}
...
int dev_usbeth_isset_write(desc_t desc){
	...
	return ofile_lst[ofile_lst[desc].desc_nxt[1]].pfsop->fdev.fdev_isset_write(USB_ENDPOINT_DESCRIPTOR_ETH_TX_NO);
}
...
int dev_usbeth_read(desc_t desc, char* buf,int cb){
	...
	g_inf_usb_eth.desc_rd = desc;
	//desc_nxt[0] is the underlying device driver read descriptor
   return ofile_lst[ofile_lst[desc].desc_nxt[0]].pfsop->fdev.fdev_read(USB_ENDPOINT_DESCRIPTOR_ETH_RX_NO,buf,cb);
}
...
int dev_usbeth_write(desc_t desc, const char* buf,int cb){
   ...
   //desc_nxt[1] is the underlying device driver write descriptor
   return ofile_lst[ofile_lst[desc].desc_nxt[1]].pfsop->fdev.fdev_write(USB_ENDPOINT_DESCRIPTOR_ETH_TX_NO,buf,cb);
}
...
int dev_usbeth_ioctl(desc_t desc,int request,va_list ap) {
   switch(request){
      ...
      case I_LINK:{
         ...
         ofile_lst[ofile_lst[desc].desc_nxt[0]].pfsop->fdev.fdev_ioctl(desc,USB_SET_ENDPOINT_TABLE,&usb_eth_virtual_ep_tbl);
         
         //set usb enumeration to driver
         ofile_lst[ofile_lst[desc].desc_nxt[0]].pfsop->fdev.fdev_ioctl(desc,USB_SET_ENUMERATION_DATA,&pusb_eth);
         
         //set usb special class function
         ofile_lst[ofile_lst[desc].desc_nxt[0]].pfsop->fdev.fdev_ioctl(desc,USB_SET_CLASS_HANDLER,&usbs_eth_class_handler);
      ...
      case USB_GET_CONTROL_ENDPOINT_STATE :{
         int *usb_eth_state = va_arg(ap, int *);
         ofile_lst[ofile_lst[desc].desc_nxt[0]].pfsop->fdev.fdev_ioctl(desc,USB_GET_CONTROL_ENDPOINT_STATE,usb_eth_state);
         return 0;
      }
      break;
...
void fill_virtual_endpoint_table(void) {
   ...
   usb_eth_virtual_ep_tbl[0]=(unsigned char *)(usb_interface_Communication_Class_Interface+USBS_CDI_ENDPOINT_OFFSET);
   usb_eth_virtual_ep_tbl[1]=(unsigned char *)(usb_interface_Data_Class_Interface_io+USB_INTERFACE_DESCRIPTOR_LENGTH);
   usb_eth_virtual_ep_tbl[2] =(unsigned char *)(usb_interface_Data_Class_Interface_io+USB_INTERFACE_DESCRIPTOR_LENGTH+USB_ENDPOINT_DESCRIPTOR_LENGTH);
}
