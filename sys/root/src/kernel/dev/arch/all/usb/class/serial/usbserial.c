...
#define  USBS_CDI_ENDPOINT_OFFSET   (USB_INTERFACE_DESCRIPTOR_LENGTH+ \
      USBS_CDC_HEADER_SIZE+USBS_CDC_CALL_MANAGEMENT_DESCRIPTOR_SIZE+ \
      USBS_CDC_ABSTRACT_CTRL_MANAGEMENT_DESCRIPTOR_SIZE+USBS_CDC_UNION_DESCRIPTOR)
#else
#define  USBS_CDI_ENDPOINT_OFFSET   (USB_INTERFACE_DESCRIPTOR_LENGTH)
#endif
...
void  dev_usbserial_timer_callback(alrm_hdl_t alarm_handle, cyg_addrword_t data) {
   if( (g_inf_usb_serial.desc_rd>=0)
      ...
      if(ofile_lst[ofile_lst[g_inf_usb_serial.desc_rd].desc_nxt[0]].pfsop->fdev.fdev_isset_read(USB_ENDPOINT_DESCRIPTOR_DC_RX_NO)){
...
int dev_usbserial_isset_read(desc_t desc){
   ...
   if(!ofile_lst[ofile_lst[desc].desc_nxt[0]].pfsop->fdev.fdev_isset_read(USB_ENDPOINT_DESCRIPTOR_DC_RX_NO)
...
int dev_usbserial_isset_write(desc_t desc){
   ...
   return ofile_lst[ofile_lst[desc].desc_nxt[1]].pfsop->fdev.fdev_isset_write(USB_ENDPOINT_DESCRIPTOR_DC_TX_NO);
}
...
int dev_usbserial_read(desc_t desc, char* buf,int cb){
   ...
   //desc_nxt[0] is the underlying device driver read descriptor
   return ofile_lst[ofile_lst[desc].desc_nxt[0]].pfsop->fdev.fdev_read(USB_ENDPOINT_DESCRIPTOR_DC_RX_NO,buf,cb);
}
...
int dev_usbserial_write(desc_t desc, const char* buf,int cb){
   ...
   //desc_nxt[1] is the underlying device driver write descriptor
   return ofile_lst[ofile_lst[desc].desc_nxt[1]].pfsop->fdev.fdev_write(USB_ENDPOINT_DESCRIPTOR_DC_TX_NO,buf,cb);
}
...
int dev_usbserial_ioctl(desc_t desc,int request,va_list ap) {
   switch(request){
      ...
      case I_LINK:{
         ...
         ofile_lst[ofile_lst[desc].desc_nxt[0]].pfsop->fdev.fdev_ioctl(desc,USB_SET_ENDPOINT_TABLE,&usb_serial_virtual_ep_tbl);
         
         //set usb enumeration to driver
         ofile_lst[ofile_lst[desc].desc_nxt[0]].pfsop->fdev.fdev_ioctl(desc,USB_SET_ENUMERATION_DATA,&pusb_serial);
         
         //set usb special class function
         ofile_lst[ofile_lst[desc].desc_nxt[0]].pfsop->fdev.fdev_ioctl(desc,USB_SET_CLASS_HANDLER,&usbs_serial_acm_class_handler);
...
#ifdef USB_CLASS_ACM
   usb_serial_virtual_ep_tbl[i++]=(unsigned char *)(usb_interface_Communication_Class_Interface+USBS_CDI_ENDPOINT_OFFSET);
#endif
   usb_serial_virtual_ep_tbl[i++]=(unsigned char *)(usb_interface_Data_Class_Interface+USB_INTERFACE_DESCRIPTOR_LENGTH);
   usb_serial_virtual_ep_tbl[i] =(unsigned char *)(usb_interface_Data_Class_Interface+USB_INTERFACE_DESCRIPTOR_LENGTH+USB_ENDPOINT_DESCRIPTOR_LENGTH);
}
