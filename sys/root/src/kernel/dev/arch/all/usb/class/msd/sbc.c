...
static unsigned char SBC_Inquiry(MSDLun *lun, usb_mass_state_t * pusb_mass_state) {
	...
	while((cb+=w)<pusb_mass_state->cmd_length) {
		if((w=usb_mass_write_payload(USB_ENDPOINT_DESCRIPTOR_DC_TX_NO,buf+cb,(int)(pusb_mass_state->cmd_length-cb)))<=0) {
			break;
		}
	}
...
static unsigned char SBC_ReadCapacity10(MSDLun *lun, usb_mass_state_t * pusb_mass_state){
	...
	while((cb+=w)<pusb_mass_state->cmd_length) {
		if((w=usb_mass_write_payload(USB_ENDPOINT_DESCRIPTOR_DC_TX_NO,buf+cb,pusb_mass_state->cmd_length-cb))<=0) {
			break;
		}
	}
		...
		switch(pusb_mass_state->cmd_state) {
			...
			case SBC_STATE_WRITE : {
				...
				while((cb+=w)<lun->blockSize) {
					if((w=usb_mass_write_payload(USB_ENDPOINT_DESCRIPTOR_DC_TX_NO,lun->readWriteBuffer+cb,lun->blockSize-cb))<=0) {
						break;
					}
				}
	  ...
	  result = MSDD_STATUS_SUCCESS;
		...
		switch (pusb_mass_state->cmd_state) {
			case SBC_STATE_READ: {
				...
				while((cb+=r)<lun->blockSize) {
					if((r=usb_mass_read_payload(USB_ENDPOINT_DESCRIPTOR_DC_RX_NO,lun->readWriteBuffer+cb,lun->blockSize-cb))<0) {
						break;
					}
				}
