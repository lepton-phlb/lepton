...
int __cb__=-1;\
   if(!(ofile_lst[__desc__].oflag&O_NONBLOCK)){\
      ...
      if(__last__){\
         ...
         __cb__ = ofile_lst[__desc__].pfsop->fdev.fdev_write(__desc__,__p_slip_write__->buf,__cb__);\
      }\
   }\
}
   ...
   #define __dev_slip_write_c(__desc_link__,__slip_desc__,__p_slip_write__,__c__,__last__) _dev_slip_write_c(__desc_link__,__c__,__last__);
...
int _dev_slip_read_c(desc_t desc, unsigned char* c,int first){
   ...
   if(!(pthread_ptr = kernel_pthread_self()))
      ...
      if(!(ofile_lst[desc].oflag&O_NONBLOCK)){
         ...
         if((p_slip_read->cb=ofile_lst[desc].pfsop->fdev.fdev_read(desc,p_slip_read->buf,SLIP_READ_BUFFER_SZ))<=0)
