...
#define WSDA(__SDA_VALUE__) (__SDA_VALUE__?((*AT91C_PIOA_SODR)=(1<<3)):((*AT91C_PIOA_CODR)=(1<<3)) )
...
----------------------------------------------*/
int _i2c_io_write(unsigned char dev_addr,const unsigned char* data,unsigned int data_len,unsigned char addr_size)
{
  char ack;
...
| Comments:  Pour la programmation de l'EEPROM verifier que \WC = 0 
----------------------------------------------*/
int _i2c_io_read(unsigned char dev_addr,unsigned char* data,unsigned int data_len,unsigned char addr_size)
{
  char ack;
  int i;
