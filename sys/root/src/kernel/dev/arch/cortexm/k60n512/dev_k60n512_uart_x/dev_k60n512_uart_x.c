...
int dev_k60n512_uart_x_write(desc_t desc, const char* buf,int size){
   ...
   HAL_WRITE_UINT8(p_uart_info->uart_base + REG_UART_D, p_uart_info->output_buffer[p_uart_info->output_r++]);
   p_uart_info->xmit=1;
...
void _kinetis_uart_x_dsr(cyg_vector_t vector, cyg_ucount32 count, cyg_addrword_t data) {
   ...
   //receive
      ...
      HAL_READ_UINT8(p_uart_info->uart_base + REG_UART_D, p_uart_info->input_buffer[p_uart_info->input_w]);
