...
static const unsigned int _kinetis_dspi_baudrate_scaler[] = { 2, 4, 6, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768 };
...
int dev_k60n512_dspi_x_load(board_kinetis_dspi_info_t * kinetis_dspi_info) {
   ...
   reg_val |= _kinetis_dspi_x_find_baudrate(/*48000000*/ CYGHWR_HAL_CORTEXM_KINETIS_CLK_PER_BUS ,kinetis_dspi_info->clk);
...
unsigned int baudrate) {
   ...
   for(pres = 0; pres < 4; pres++) {
      for(scaler = 0; scaler < 16; scaler++) {
         for(dbr = 1; dbr < 3; dbr++) {
            diff = baudrate - ((bus_clk * dbr) / (_kinetis_dspi_baudrate_prescaler[pres] * (_kinetis_dspi_baudrate_scaler[scaler])));
            if(diff < 0)
               diff = -diff;
