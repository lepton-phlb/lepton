...
//
static ep0_low_level_status_t _at91_usbdp_control_error(board_inf_usb_t *p_inf_usb, ep0_low_level_status_t status);
static ep0_low_level_status_t _at91_usbdp_control_data_recv(board_inf_usb_t *p_inf_usb, ep0_low_level_status_t status);
static ep0_low_level_status_t _at91_usbdp_control_data_sent(board_inf_usb_t *p_inf_usb, ep0_low_level_status_t status);
static ep0_low_level_status_t _at91_usbdp_control_setup(board_inf_usb_t *p_inf_usb, ep0_low_level_status_t status);
...
| See:
---------------------------------------------*/
void _at91_usbdp_ep_interrupt_enable (board_inf_usb_t *p_inf_usb, unsigned char epn, unsigned char enable) {
   if(!p_inf_usb) {
      return;
   }
...
| See:
---------------------------------------------*/
void _at91_usbdp_ep_init(board_inf_usb_t *p_inf_usb, usbs_data_endpoint * pep, unsigned char endpoint_type, bool_t enable) {
...
void _at91_usbdp_reset_device(board_inf_usb_t *p_inf_usb) {
   ...
   USBDP_WRITE_UINT32(pCSRn((p_inf_usb->base_addr + UDP_CSRx),0),AT91C_UDP_EPEDS | AT91C_UDP_EPTYPE_CTRL);
...
void _at91_usbdp_handle_reset(board_inf_usb_t *p_inf_usb) {
   ...
   for(i=0;;i++) {
      ...
      if(endpoint_number < p_inf_usb->ep_no ) {
         _at91_usbdp_ep_init(p_inf_usb, (usbs_data_endpoint *)p_inf_usb->ep_tbl[endpoint_number], endpoint_type, USB_TRUE);
      }
   }
}
...
| See:
---------------------------------------------*/
unsigned char * _at91_usbdp_read_fifo_uint8(unsigned char  * pdest, unsigned int psource, unsigned int size) {
...
| See:
---------------------------------------------*/
unsigned char * _at91_usbdp_write_fifo_uint8(unsigned int pdest, unsigned char * psource, unsigned char * psource_end) {
...
unsigned char _at91_usbdp_ep_isr_tx(board_inf_usb_t *p_inf_usb, unsigned char epn) {
   ...
   if (BITS_ARE_CLEARED (pCSR, AT91C_UDP_TXPKTRDY )) {
      ...
      if (*ppend > *ppbegin) {
         ...
         //
         *ppbegin =
               _at91_usbdp_write_fifo_uint8 (pFDR, *ppbegin, (unsigned char *) ((unsigned int) * ppbegin +
...
void _at91_usbdp_control_dsr(board_inf_usb_t *p_inf_usb) {
   ...
   while (!BITS_ARE_CLEARED((unsigned int)(p_inf_usb->base_addr + UDP_CSRx),
      ...
      if (BITS_ARE_CLEARED ((unsigned int)(p_inf_usb->base_addr + UDP_CSRx), AT91C_UDP_TXPKTRDY )) {
...
| See:
---------------------------------------------*/
ep0_low_level_status_t _at91_usbdp_control_error(board_inf_usb_t *p_inf_usb, ep0_low_level_status_t status) {
...
| See:
---------------------------------------------*/
ep0_low_level_status_t _at91_usbdp_control_setup(board_inf_usb_t *p_inf_usb, ep0_low_level_status_t status) {
   unsigned char **ppbegin = &p_inf_usb->ep_buffer_pbegin[0];
   unsigned char **ppend = &p_inf_usb->ep_buffer_pend[0];
   usbs_control_endpoint * at91_usbdp_ep0 = (usbs_control_endpoint *)p_inf_usb->ep_tbl[0];
   usb_devreq *req = (usb_devreq *) at91_usbdp_ep0->control_buffer;
   ...
   _at91_usbdp_read_fifo_uint8 ((unsigned char *)req, (unsigned int)(p_inf_usb->base_addr + UDP_FDRx), sizeof (usb_devreq));
...
| See:
---------------------------------------------*/
ep0_low_level_status_t _at91_usbdp_control_data_recv(board_inf_usb_t *p_inf_usb, ep0_low_level_status_t status) {
   ...
   if (status == EP0_LL_RECEIVE_READY) {
      ...
      *ppbegin = _at91_usbdp_read_fifo_uint8(*ppbegin, (unsigned int)(p_inf_usb->base_addr + UDP_FDRx)/*(unsigned int)pFDR0*/, length);
...
| See:
---------------------------------------------*/
ep0_low_level_status_t _at91_usbdp_control_data_sent(board_inf_usb_t *p_inf_usb, ep0_low_level_status_t status) {
   unsigned char **ppbegin = &p_inf_usb->ep_buffer_pbegin[0];
   unsigned char **ppend = &p_inf_usb->ep_buffer_pend[0];
   ...
   switch (status) {
      ...
      case EP0_LL_SEND_READY:
         if (*ppbegin == *ppend &&
            ...
            *ppbegin = _at91_usbdp_write_fifo_uint8 ((unsigned int)(p_inf_usb->base_addr + UDP_FDRx), *ppbegin, (unsigned char *)
