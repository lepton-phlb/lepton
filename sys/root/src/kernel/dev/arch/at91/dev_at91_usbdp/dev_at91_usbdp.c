...
cyg_uint32 dev_at91_usbdp_isr(cyg_vector_t vector, cyg_addrword_t data) {
   ...
   if(BITS_ARE_CLEARED ((unsigned int)(p_inf_usb->base_addr + UDP_ISR), AT91_UDP_ALLOWED_IRQs & 0xffffff01)
         && !need_dsr) {
...
void dev_at91_usbdp_dsr(cyg_vector_t vector, cyg_ucount32 count, cyg_addrword_t data) {
   ...
   if(BITS_ARE_SET ((unsigned int)(p_inf_usb->base_addr + UDP_ISR), AT91C_UDP_ENDBUSRES)) {
      ...
      //loop
      USBDP_WRITE_UINT32((unsigned int)(p_inf_usb->base_addr + UDP_CSRx), AT91C_UDP_EPEDS | AT91C_UDP_EPTYPE_CTRL);
...
int dev_at91_usbdp_open(desc_t desc, int o_flag) {
   ...
   if(p_g_board_usb->opened) {
      ...
      //create ISR and DSR
      cyg_interrupt_create((cyg_vector_t)p_g_board_usb->irq_no, (cyg_priority_t)p_g_board_usb->irq_prio, (cyg_addrword_t)p_g_board_usb,
