...
int _fat16_vfat_lookupname_ud(fat16_core_info_t * fat_info, desc_t desc_new, char* filename) {
   ...
   while(!found) {
      ...
      while((i<((fat_info->nbsec_per_clus * FAT16_BS_BPS_VAL)/RD_SIZE))&&(!found)) {
         if(_fat_read_data(__get_dev_desc(desc_new),addr,(unsigned char *)&fat_entry.msdos_entry,RD_SIZE)<0) {
            return -1;
         }
         ...
         if((fat_entry.vfat_entry.LDIR_Ord&FAT16_LAST_LONGENTRY_MSK)
            ...
            //rewind offset
            ofile_lst[__get_dev_desc(desc_new)].pfsop->fdev.fdev_seek(__get_dev_desc(desc_new),addr-RD_SIZE,SEEK_SET);
            nb_ent = __vfat_get_nb_long_entries(fat_entry.vfat_entry);

            _fat16_vfat_build_oob_name(fat_info, __get_dev_desc(desc_new), longname, nb_ent, cluster);
            //entry found
   ...
   //
   _fat16_vfat_get_infos_addr(desc_new, cluster, (fat_entry.vfat_entry.LDIR_Ord&~FAT16_LAST_LONGENTRY_MSK),&addr);
...
int _fat16_vfat_build_name(desc_t dev_desc, unsigned char * dst_longname, int nb_ent) {
   ...
   for(i=0;i<nb_ent;i++) {
      ...
      _fat16_vfat_extract_name(dst_longname+i*FAT16_LONGNAME_SUM,(const fat_vfat_dir_entry_t *)&vfat_entry);
   }
   return 0;
}
...
int _fat16_vfat_build_oob_name(fat16_core_info_t * fat_info, desc_t dev_desc, unsigned char * dst_longname, int nb_ent, fat16_u16_t cluster) {
   ...
   nb_entry_close = ((cluster_addr+fat_info->nbsec_per_clus * FAT16_BS_BPS_VAL-RD_SIZE)-current_addr)/RD_SIZE;
...
int _fat16_vfat_get_infos_addr(desc_t desc_new, fat16_u16_t cluster, int nb_entries, fat16_u32_t *current_addr) {
   ...
   cluster_addr=_fat16_cluster_add(ofile_lst[desc_new].pmntdev->fs_info.fat_info.fat_core_info, cluster);
   nb_ent_close = ((cluster_addr+(ofile_lst[desc_new].pmntdev->fs_info.fat_info.fat_boot_info->bpb.BPB_SecPerClust*ofile_lst[desc_new].pmntdev->fs_info.fat_info.fat_boot_info->bpb.BPB_BytesPerSec)-RD_SIZE)-*current_addr)/RD_SIZE;

   //maybe entry is splitted between 2 clusters
   if(nb_ent_close<nb_entries) {
      cluster_suiv = _fat16_cluster_suiv(ofile_lst[desc_new].pmntdev->fs_info.fat_info.fat_core_info, __get_dev_desc(desc_new), cluster);
      cluster_addr = _fat16_cluster_add(ofile_lst[desc_new].pmntdev->fs_info.fat_info.fat_core_info, cluster_suiv);
...
fat16_u32_t _fat16_vfat_get_addr_dot_items(fat16_core_info_t * fat_info, desc_t desc, fat16_u16_t parent_cluster, fat16_u16_t pattern_cluster) {
   ...
   if(_fat_read_data(__get_dev_desc(desc),current_addr+RD_SIZE,(unsigned char *)&fat_entry.msdos_entry,RD_SIZE)<0) {
      return 0;
   }

   cluster = fat_entry.msdos_entry.DIR_FstClusLO[0] + ((fat16_u16_t)fat_entry.msdos_entry.DIR_FstClusLO[1]<<8);
   //2 cases
   //dot is in rootdir
   if(!cluster) {
      ...
      while((j<((fat_info->nbsec_per_clus * FAT16_BS_BPS_VAL)/RD_SIZE))) {
         if(_fat_read_data(__get_dev_desc(desc),current_addr,(unsigned char *)&fat_entry.vfat_entry,RD_SIZE)<0) {
            return 0;
         }
         ...
         if (fat_entry.msdos_entry.DIR_Name[0]!= '.'
               && (pattern_cluster == (fat_entry.msdos_entry.DIR_FstClusLO[0] + ((fat16_u16_t)fat_entry.msdos_entry.DIR_FstClusLO[1]<<8)))) {
            return return_addr;
         }
         //
         return_addr = current_addr;
...
int _fat16_vfat_get_next_long_entry(desc_t desc_new, fat16_u16_t *cluster, int nb_entries, fat16_u32_t *current_addr) {
   ...
   if(nb_ent_close<nb_entries) {
      ...
      if(!(cluster_suiv = _fat16_cluster_suiv(ofile_lst[desc_new].pmntdev->fs_info.fat_info.fat_core_info, __get_dev_desc(desc_new), *cluster))
...
int _fat16_vfat_request_entries(desc_t desc, fat16_u32_t addr_begin, fat16_u32_t addr_end, int nb_ent) {
   ...
   for(;i<nb_ent;i++) {
      if(_fat_read_data(__get_dev_desc(desc),addr_begin,(unsigned char *)&fat_entry.msdos_entry.DIR_Name[0],1)<0) {
         return -1;
      }

      //not enough
      if((fat_entry.msdos_entry.DIR_Name[0]!=RD_ENT_FREE && fat_entry.msdos_entry.DIR_Name[0]!=RD_ENT_REMOVE)
            || addr_begin == addr_end) {
         return i;
      }
      addr_begin += RD_SIZE;
...
int _fat16_vfat_remove_entries(fat16_core_info_t * fat_info, desc_t desc) {
   ...
   //read entry
   if(_fat_read_data(__get_dev_desc(desc),fat16_ofile_lst[desc].entry_phys_addr,(unsigned char *)&fat_entry,RD_SIZE)<0) {
      return -1;
   }
   ...
   if(_ino_phys<__get_BPB_RootEntCnt(ofile_lst[desc].pmntdev)) {
      ...
      //remove long entries
      for(;i<nb_entries;i++) {
         if(_fat_write_data(__get_dev_desc(desc),fat16_ofile_lst[desc].entry_phys_addr+i*RD_SIZE,&rem_pattern,1)<0) {
            return -1;
         }
      }
   }
   //or user data
      ...
      fat16_u16_t current_cluster = (fat16_ofile_lst[desc].entry_phys_addr-__get_ud_addr(ofile_lst[desc].pmntdev))/
      ...
      int nb_entry_close = ((current_addr+fat_info->nbsec_per_clus * FAT16_BS_BPS_VAL)-fat16_ofile_lst[desc].entry_phys_addr)/RD_SIZE;
      ...
      if(nb_entry_close>=nb_entries) {
         ...
         fat16_u16_t cluster_suiv=_fat16_cluster_suiv(fat_info, __get_dev_desc(desc), current_cluster);
...
inodenb_t _fat16_vfat_request_free_rd(desc_t desc, fat16_u32_t * addr, int req_ent) {
   ...
   } while ((fat_entry.msdos_entry.DIR_Name[0]!=RD_ENT_FREE && fat_entry.msdos_entry.DIR_Name[0]!=RD_ENT_REMOVE)
         || avail_ent);
...
inodenb_t _fat16_vfat_request_free_ud(desc_t desc, fat16_u32_t * addr, fat16_u16_t * cluster, fat16_u32_t *prev_addr, fat16_u16_t * prev_cluster, int req_ent, int * rem_ent) {
      ...
      while(1) {
         ...
         if((/*fat_entry.vfat_entry.LDIR_Ord&FAT16_LAST_LONGENTRY_MSK)*/
            ...
            *prev_cluster = *cluster;
            ...
            if(!(*addr = _fat16_vfat_get_next_long_entry(desc, cluster, (fat_entry.vfat_entry.LDIR_Ord&~FAT16_LAST_LONGENTRY_MSK)+1,addr))) {
               //allocate a new cluster
               if(!(*cluster=_fat16_getclus(ofile_lst[desc].pmntdev->fs_info.fat_info.fat_core_info, __get_dev_desc(desc), &ofile_lst[desc].pmntdev->fs_info.fat_info.fat_boot_info->bpb))) {
                  //FAT is full
                  return INVALID_INODE_NB;
               }
               //
               _fat16_chainclus(ofile_lst[desc].pmntdev->fs_info.fat_info.fat_core_info, __get_dev_desc(desc), *prev_cluster, *cluster);
            ...
            *prev_cluster = *cluster;
            ...
            if(*prev_cluster == *cluster) {
               cluster_addr = _fat16_cluster_add(ofile_lst[desc].pmntdev->fs_info.fat_info.fat_core_info, *cluster);
               avail_ent = _fat16_vfat_request_entries(desc, *addr, cluster_addr+__get_nbsec_per_clus(ofile_lst[desc].pmntdev) * FAT16_BS_BPS_VAL,
                     req_ent);
               *rem_ent = req_ent-avail_ent;
               ...
               if(!avail_ent) {
                  ...
                  fat16_u16_t cluster_suiv=_fat16_cluster_suiv(ofile_lst[desc].pmntdev->fs_info.fat_info.fat_core_info,  __get_dev_desc(desc), *cluster);
                  //last cluster
                  ...
                  //left space in next cluster?
                     ...
                     cluster_suiv_addr = _fat16_cluster_add(ofile_lst[desc].pmntdev->fs_info.fat_info.fat_core_info, cluster_suiv);
                     avail_ent2 = _fat16_vfat_request_entries(desc, cluster_suiv_addr, cluster_suiv_addr+__get_nbsec_per_clus(ofile_lst[desc].pmntdev) * FAT16_BS_BPS_VAL,
                           *rem_ent);
               ...
               if(!*cluster) {
                  ...
                  *addr = _fat16_cluster_add(ofile_lst[desc].pmntdev->fs_info.fat_info.fat_core_info, *cluster);
                  ...
                  cluster_addr = _fat16_cluster_add(ofile_lst[desc].pmntdev->fs_info.fat_info.fat_core_info, *prev_cluster);
                  avail_ent = _fat16_vfat_request_entries(desc, *prev_addr, cluster_addr+__get_nbsec_per_clus(ofile_lst[desc].pmntdev) * FAT16_BS_BPS_VAL,
                        req_ent);

                  *rem_ent = req_ent-avail_ent;
                  //available entries in next cluster
                  cluster_addr = _fat16_cluster_add(ofile_lst[desc].pmntdev->fs_info.fat_info.fat_core_info, *cluster);
                  avail_ent2 = _fat16_vfat_request_entries(desc, *addr, cluster_addr+__get_nbsec_per_clus(ofile_lst[desc].pmntdev) * FAT16_BS_BPS_VAL,
                        *rem_ent);
   ...
   return __get_BPB_RootEntCnt(ofile_lst[desc].pmntdev)+(*addr-__get_ud_addr(ofile_lst[desc].pmntdev))/RD_SIZE;
}
/*============================================
