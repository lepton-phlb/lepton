                     ...
                     NGLXContext ctx1)
{
  ...
  GR_SCREEN_INFO scr_info;

  if (ctx->gl_context == NULL) {
      /* create the TinyGL context */
      GrGetWindowInfo(drawable, &win_info);
      GrGetScreenInfo(&scr_info);


      xsize = win_info.width;
      ysize = win_info.height;

      switch(scr_info.pixtype) {
         case MWPF_PALETTE: {
            unsigned int palette[255];
            unsigned char color_indexes[255];
            unsigned long pixel[255];
            GR_PALETTE pal={0};
            int i=0;
            ...
            for(i=0;i<pal.count;i++) {
               pixel[i] = (unsigned long) GrGetSysColor(i);
               //
               color_indexes[i]=(unsigned char)pixel[i];
            }

            mode = ZB_MODE_INDEX;//ZB_MODE_5R6G5B;
            zb=ZB_open(xsize,ysize,mode,pal.count,color_indexes,palette,NULL);
            if (zb == NULL) {
               fprintf(stderr, "Error while initializing Z buffer\n");
               exit(1);
            }

            ctx->pixtype = MWPF_PALETTE;//MWPF_TRUECOLOR565;
         }
         break;
